...
def find_earliest_timestamp(
    urls: list[str], posts: list[dict], bookmarks: list[dict]
) -> str | None:
...
def main():
    ...
    parser.add_argument(
        "--urls", nargs="+", help="Find earliest timestamp for specific URLs"
    )
    ...
    if args.urls:
        ...
        earliest = find_earliest_timestamp(
            args.urls, mastodon_posts, linkding_bookmarks
        )
        if earliest:
        ...
        if args.json:
            ...
            for bookmark in linkding_bookmarks[:5]:  # Show first 5
                timestamp = (
                    bookmark["timestamp"] if bookmark["timestamp"] else "No timestamp"
                )
