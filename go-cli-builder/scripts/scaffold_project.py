...
def create_directory_structure(
    project_path, include_database=True, include_templates=False
):
...
def copy_template(template_name, dest_path, replacements=None, modify_content=None):
    ...
    print(
        f"✅ Created: {dest_path.relative_to(dest_path.parents[len(dest_path.parents) - 2])}"
    )
    return True


def scaffold_project(
    project_name, output_dir=".", include_database=True, include_templates=False
):
    ...
    def remove_database_from_root(content):
        ...
        for line in lines:
            if (
                "Database flag" in line
                or "database" in line
                and "rootCmd.PersistentFlags()" in line
            ):
                continue
    ...
    def remove_cgo_from_makefile(content):
        ...
        for line in lines:
            if (
                "SQLite requires CGO" in line
                or "static linking" in line.lower()
                and "sqlite" in line.lower()
            ):
    ...
    copy_template(
        "config.yaml.example",
        project_path / f"{project_name}.yaml.example",
        replacements,
    )
    ...
    copy_template(
        "root.go.template",
        project_path / "cmd" / "root.go",
        replacements,
        modify_content=None if include_database else remove_database_from_root,
    )

    copy_template(
        "version.go.template", project_path / "cmd" / "version.go", replacements
    )
    copy_template(
        "constants.go.template", project_path / "cmd" / "constants.go", replacements
    )

    # config.go - conditionally include Database field
    copy_template(
        "config.go.template",
        project_path / "internal" / "config" / "config.go",
        replacements,
    ...
    copy_template(
        "rolling-release.yml.template",
        project_path / ".github" / "workflows" / "rolling-release.yml",
        replacements,
    )
    ...
    if include_database:
        copy_template(
            ...
            project_path / "internal" / "database" / "database.go",
            replacements,
        )
        copy_template(
            "migrations.go.template",
            project_path / "internal" / "database" / "migrations.go",
            replacements,
        )
        copy_template(
            "schema.sql.template",
            project_path / "internal" / "database" / "schema.sql",
            replacements,
        )
    ...
    if include_templates:
        copy_template(
            "init.go.template", project_path / "cmd" / "init.go", replacements
        )
        copy_template(
            "templates.go.template",
            project_path / "internal" / "templates" / "templates.go",
            replacements,
        )
        copy_template(
            "default.md.template",
            project_path / "internal" / "templates" / "default.md",
            replacements,
        )
    ...
    print(
        "3. Update GitHub Actions workflows with your Docker Hub username (if needed)"
    )
...
def main():
    ...
    parser.add_argument(
        "--no-database", action="store_true", help="Exclude database support (SQLite)"
    )
    parser.add_argument(
        "--templates",
        action="store_true",
        help="Include template support (for generating formatted output)",
    )
