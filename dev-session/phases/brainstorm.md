...
2. **Codebase research substep** (blank-slate only). Dispatch a documentarian-mode subagent (typically `Explore` or `general-purpose`) to answer 3-5 neutral questions about how the relevant areas of the codebase currently work. Frame the questions in terms of existing components and flows, not the desired feature — the subagent should describe what exists, not propose how to change it.
   ...
   - **Class-of-bug analogues.** When the task framing is "fix X for set Y" (e.g., "fix this filter for these three roles"), include a question about other code paths through X — analogues that aren't in Y but might share the bug shape. Surfacing them at research time costs one extra question; surfacing them at PR-review time costs a follow-up issue or scope creep. Document the answer even if you decide to leave the analogues out of the spec — your `What we're NOT doing` section should reference them explicitly.
   - **Generic / catch-all consumers.** When adding a new entity to a system that has a generic handler — a registry, an `unknown-X`/`default:` branch, a catch-all heuristic that processes "anything not explicitly known" — include a question: *what will automatically pick up this new thing, intended or not?* New actions/types/events often get silently consumed (or silently ignored) by such code. Surfacing it at research time is one question; missing it ships wrong behavior that passes tests and aggregate metrics (it only shows in a targeted/verbose run). Note the consumers in the spec's patterns-to-follow.
...
3. **Interactive Q&A** (blank-slate only). Ask one question at a time to develop the spec, grounded in `research.md`. Each question should build on previous answers. Propose your best judgment with the trade-offs from research, and ask to confirm or adjust — don't ask open-ended questions when you have a clear recommendation. Multiple choice preferred when feasible.
   - **Interrogate from both sides — mechanical *and* in-world.** Don't only ask "how should this work?" (implementation, data, edge cases); also ask "does this make sense diegetically / for the player's mental model?" For product/game/UX work the in-world question often reshapes the mechanic for the better (e.g., "why would this entity offer that?" can turn a generic feature into a grounded one). Treat the fiction/feel lens as a first-class design check, not a tangent.
...
4. **Keep proportional to complexity.** For small, well-defined issues, 2-4 questions covering key design decisions is enough. For larger or ambiguous issues, go deeper. If the design is obvious after a couple of questions, say so and offer to move to the spec.
   - **Flag feel-driven features.** If the feature is dominated by subjective visual / animation / interaction *feel* (shapes, motion, easing, layout density, juice) that a spec genuinely can't pin down, name that now and plan an **interactive-prototype phase** instead of routing to autonomous `execute`/`express` — see "Feel-driven features want a prototype" in SKILL.md. Tell: if the spec wants to say "tune by eye" or "TBD, adjust later," it's feel-driven, and a disposable slider-driven harness (gitignored `tmp/`) iterated with the user will converge far faster than spec → autonomous build.
